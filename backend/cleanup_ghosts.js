...
mongoose.connect('mongodb://localhost:27017/room-Fio').then(async () => {
  ...
  const validRoomIds = allRooms.map(k => k._id.toString());
  
  let deletedCount = 0;
  for (let r of allRentals) {
    if (!r.roomId || !validRoomIds.includes(r.roomId.toString()) || badCustIds.some(id => id.toString() === r.customerId.toString())) {
